...
contrib_dataset = np.load("D:/project/data/beijing/small_domain/contribution_analysis.npy") #(365, 21)各个特征的单位都是%贡献度，因此不需标准化
...
def APCluster(preference=-2000):
    ...
    for i in range(len(APmodel.cluster_centers_)):
        ...
        plt.scatter(d[0],d[1],s=50)
    plt.savefig('D:/project/data/beijing/small_domain/cluster_result/APCluster/'+'APCluster_'+str(preference)+'_var'+'.png')
...
def KMCluster(n_clusters):
    ...
    for i in range(len(KMmodel.cluster_centers_)):
        d = tsne2[var_dataset_revised['cluster_label'] == i]  #找出聚类类别为i的数据对应的降维结果
        plt.scatter(d[0],d[1],s=50)
    plt.savefig('D:/project/data/beijing/small_domain/cluster_result/KMCluster/'+'KMCluster_'+str(n_clusters)+'_var'+'.png')
    #plt.show()
