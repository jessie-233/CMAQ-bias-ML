...
np.random.seed(42)
file = np.load("E:/project/beijing/dataset.npy")
...
#计算测试集RMSE损失
y_pred = model.predict(X_test) #归一化后的
y_pred = scaler2.inverse_transform(y_pred) #归一化前，相对误差
y_test = scaler2.inverse_transform(y_test) #归一化前，相对误差
test_rmse = math.sqrt(metrics.mean_squared_error(y_test, y_pred))
print('测试集RMSE损失为：%.3f' % test_rmse)


#计算修正后的各项指标
#bias=sim-obs/obs,PM25_revised=sim/(1+bias)
...
def cal_rmse(xtest, ytest = None, info = ''):
    if ytest is None:
        ...
        PM25_revised = ytest[:,var_dict.get('PM2.5_Sim')] / (1 + y_pred.reshape(len(xtest),))
