...
for seed in seeds:
    ...
    for m in mete_bias:
        ...
        row_1 += 1
    writer = pd.ExcelWriter('D:/project/data/BTH/lessvar/feature_selection/feature_selection_temp.xlsx',engine='openpyxl')
    book = load_workbook('D:/project/data/BTH/lessvar/feature_selection/feature_selection_temp.xlsx')
    writer.book = book
    ...
    while True:
        ...
        if 'WSPD_Bias' in var_base and 'WSPD_Obs' in var_obs:
            var_obs.remove('WSPD_Obs')
        if 'PRE_Bias' in var_base and 'PRE_Obs' in var_obs:
            var_obs.remove('PRE_Obs')
        if 'PBLH_Sim' in var_base and 'SOLRAD_Sim' in var_obs:
            var_obs.remove('SOLRAD_Sim')
        if 'SOLRAD_Sim' in var_base and 'PBLH_Sim' in var_obs:
            var_obs.remove('PBLH_Sim')
