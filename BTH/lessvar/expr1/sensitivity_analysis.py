...
#加载训练好的model
DNN_model = load_model("D:/project/data/BTH/new/model2/DNN_model2.h5")
'''
#制作sheets
#bj_detail
...
#写入excel
writer = pd.ExcelWriter("D:/project/data/BTH/new/model2/contribution_analysis_temp.xlsx")
...
x_sample = datasetX_bj[354,:].reshape((-1,datasetX_bj.shape[1])) #low PM2.5_Bias_ystd
print(x_sample)

# [[-3.26 -1.58  0.89 -0.18 -2.14  0.63  1.19  0.84 -0.07]]

y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.2 #加delta 从0.01往上加，至0.2有值，第2个变量贡献度100%
...
x_sample = datasetX_bj[356,:].reshape((-1,datasetX_bj.shape[1])) #low PM2.5_Bias_ystd
print(x_sample)

# [[-1.65 -2.7   1.58  0.04 -3.03  0.43  0.54 -0.19 -0.07]]

y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.4 #加delta 从0.01往上加，至0.4有值，第1个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
    x_sample[0,i] = x_sample[0,i] - 0.4 #恢复
    #print("取第{0}条数据，第{1}个变量上调{2}，y增加{3}".format(332, i, 0.01, y[i] - y_pred))
sum_delt = sum(abs(y - y_pred))
for i in range(x_sample.shape[1]):
    result[i] = (abs(y[i] - y_pred) / sum_delt) * 100
...
x_sample = datasetX_bj[357,:].reshape((-1,datasetX_bj.shape[1])) #low PM2.5_Bias_ystd
print(x_sample)

# [[-7.87 -2.12  1.19  0.1  -1.18 -0.04  0.85 -0.12 -0.07]]

y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 1.11 #加delta 从0.01往上加，至1.11有值，第5个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
    x_sample[0,i] = x_sample[0,i] - 1.11 #恢复
    #print("取第{0}条数据，第{1}个变量上调{2}，y增加{3}".format(332, i, 0.01, y[i] - y_pred))
sum_delt = sum(abs(y - y_pred))
for i in range(x_sample.shape[1]):
    result[i] = (abs(y[i] - y_pred) / sum_delt) * 100
