...
for var in var_sele:
    ...
    i += 1
scaler1 = preprocessing.StandardScaler().fit(X) #特征缩放（Feature scaling）：求z-score的方法，将样本的特征值转换到同一量纲下
...
#加载训练好的model
DNN_model = load_model("D:/project/data/BTH/allvar/expr3/DNN_model3.h5")
...
#写入excel
writer = pd.ExcelWriter("D:/project/data/BTH/allvar/expr3/sensitivity_analysis_temp.xlsx")
...
#datasetX_bj[277,:]
print(get_contribution(datasetX_bj, 277, DNN_model)) #除以了0 RuntimeWarning: invalid value encountered in double_scalars
x_sample = datasetX_bj[277,:].reshape((-1,datasetX_bj.shape[1])) #low in PM2.5_Bias_ystd
print(x_sample)

# [[ 1.11 -3.54 -0.19  0.86  0.1   0.64 -0.87  0.28 -2.11  0.26  0.67  0.31
#    0.17  0.9   0.56 -0.73 -0.24 -0.28 -0.17]]

y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.3 #加delta 从0.01往上加，至0.3有值，倒数第5个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
    x_sample[0,i] = x_sample[0,i] - 0.3 #恢复
...
#datasetX_bj[331,:]
print(get_contribution(datasetX_bj, 331, DNN_model)) #除以了0 RuntimeWarning: invalid value encountered in double_scalars
x_sample = datasetX_bj[331,:].reshape((-1,datasetX_bj.shape[1])) #low in PM2.5_Bias_ystd
print(x_sample)

# [[ 0.66 -2.11 -2.27  0.31  0.7   2.86 -0.29 -1.36  0.25  0.   -0.02  0.47
#   -0.06  1.18 -1.43 -1.04 -0.24 -1.31 -0.94]]

y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.2 #加delta 从0.01往上加，至0.2有值，第3个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
    x_sample[0,i] = x_sample[0,i] - 0.2 #恢复
    #print("取第{0}条数据，第{1}个变量上调{2}，y增加{3}".format(332, i, 0.01, y[i] - y_pred))
sum_delt = sum(abs(y - y_pred))
for i in range(x_sample.shape[1]):
    result[i] = (abs(y[i] - y_pred) / sum_delt) * 100
...
#datasetX_bj[354,:]
print(get_contribution(datasetX_bj, 354, DNN_model)) #除以了0 RuntimeWarning: invalid value encountered in double_scalars
x_sample = datasetX_bj[354,:].reshape((-1,datasetX_bj.shape[1])) #low in PM2.5_Bias_ystd
print(x_sample)

# [[ 0.63 -3.26 -1.58  0.89 -0.18  2.16 -0.43 -1.05 -2.14  0.63  1.19  0.84
#   -0.07  0.69 -1.39 -0.9  -0.24 -1.11 -1.05]]

y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.16 #加delta 从0.01往上加，至0.16有值，倒数第3个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
    x_sample[0,i] = x_sample[0,i] - 0.16 #恢复
    #print("取第{0}条数据，第{1}个变量上调{2}，y增加{3}".format(332, i, 0.01, y[i] - y_pred))
sum_delt = sum(abs(y - y_pred))
for i in range(x_sample.shape[1]):
    result[i] = (abs(y[i] - y_pred) / sum_delt) * 100
...
x_sample = datasetX_bj[356,:].reshape((-1,datasetX_bj.shape[1])) #low in PM2.5_Bias_ystd
print(x_sample)
...
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.5 #加delta 从0.01往上加，至0.5有值，倒数第3个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
