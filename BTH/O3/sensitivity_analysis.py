...
#异常值：sum_delta=0，说明delta太小
# #datasetX_bj[331,:]
# print(get_contribution(datasetX_bj, 331, DNN_model)) #除以了0 RuntimeWarning: invalid value encountered in double_scalars
# x_sample = datasetX_bj[331,:].reshape((-1,datasetX_bj.shape[1])) #nothing special, but high PM25 bias (y)
# print(x_sample)
# '''
# [[ 0.66 -2.11 -2.27  0.31  0.7   2.86 -0.29 -1.36  0.25  0.    0.57  0.03
#   -0.06  1.18 -1.43 -0.06 -0.24 -1.31 -0.94]]
# '''
# y_pred = float(DNN_model.predict(x_sample))
# y = np.zeros(x_sample.shape[1])
# result = np.zeros(x_sample.shape[1])
# for i in range(x_sample.shape[1]):
#     x_sample[0,i] = x_sample[0,i] + 0.05 #加delta 从0.01往上加，至0.05有值，倒数第三个变量贡献度100%
#     y[i] = float(DNN_model.predict(x_sample))
#     x_sample[0,i] = x_sample[0,i] - 0.05 #恢复
#     #print("取第{0}条数据，第{1}个变量上调{2}，y增加{3}".format(332, i, 0.01, y[i] - y_pred))
# sum_delt = sum(abs(y - y_pred))
# for i in range(x_sample.shape[1]):
#     result[i] = (abs(y[i] - y_pred) / sum_delt) * 100
...
# #datasetX_bj[356,:]
# print(get_contribution(datasetX_bj, 356, DNN_model)) #除以了0 RuntimeWarning: invalid value encountered in double_scalars
# x_sample = datasetX_bj[356,:].reshape((-1,datasetX_bj.shape[1])) #high in NO2_Obs,low in RH_Bias
# print(x_sample)
# '''
# [[ 0.1  -1.65 -2.7   1.58  0.04  3.35 -0.74 -1.32 -3.03  0.43  0.57  0.02
#   -0.07  1.16 -1.57 -0.04 -0.24 -1.24 -1.01]]
# '''
# y_pred = float(DNN_model.predict(x_sample))
# y = np.zeros(x_sample.shape[1])
# result = np.zeros(x_sample.shape[1])
# for i in range(x_sample.shape[1]):
#     x_sample[0,i] = x_sample[0,i] + 0.02 #加delta 从0.01往上加，至0.02有值，倒数第三个变量贡献度100%
#     y[i] = float(DNN_model.predict(x_sample))
#     x_sample[0,i] = x_sample[0,i] - 0.02 #恢复
#     #print("取第{0}条数据，第{1}个变量上调{2}，y增加{3}".format(332, i, 0.01, y[i] - y_pred))
# sum_delt = sum(abs(y - y_pred))
# for i in range(x_sample.shape[1]):
#     result[i] = (abs(y[i] - y_pred) / sum_delt) * 100
...
#datasetX_bj[357,:]
print(get_contribution(datasetX_bj, 357, DNN_model)) #除以了0 RuntimeWarning: invalid value encountered in double_scalars
x_sample = datasetX_bj[357,:].reshape((-1,datasetX_bj.shape[1])) #low in PM2.5_Bias_ystd
print(x_sample)
'''
[[-0.1  -7.87 -2.12  1.19  0.1   2.37 -0.8  -1.33 -1.18 -0.04  0.57  0.02
  -0.07  0.71 -1.45 -0.02 -0.24 -0.99 -1.03]]
'''
y_pred = float(DNN_model.predict(x_sample))
y = np.zeros(x_sample.shape[1])
result = np.zeros(x_sample.shape[1])
for i in range(x_sample.shape[1]):
    x_sample[0,i] = x_sample[0,i] + 0.1 #加delta 从0.01往上加，至0.1有值，倒数第三个变量贡献度100%
    y[i] = float(DNN_model.predict(x_sample))
...
# #写入excel
# writer = pd.ExcelWriter("D:/project/data/BTH/new/contribution_analysis_new.xlsx")
