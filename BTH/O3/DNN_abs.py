...
plt.savefig("D:/project/data/BTH/O3/DNN_Model_Loss.png")
# plt.subplot(1, 2, 2)
...
#计算修正后的各项指标
#bias=sim-obs,O3_revised=sim-bias

#RMSE
def cal_rmse(xtest, ytest = None, info = ''):
    if ytest is None:
        RMSE = math.sqrt(metrics.mean_squared_error(xtest[:,var_dict.get('O3_Obs')], xtest[:,var_dict.get('O3_Sim')]))
    else:
        y_pred = model.predict(xtest)
        y_pred = scaler2.inverse_transform(y_pred) #计算winter时用全集的scaler
        O3_revised = ytest[:,var_dict.get('O3_Sim')] - y_pred.reshape(len(xtest),)
        RMSE = math.sqrt(metrics.mean_squared_error(ytest[:,var_dict.get('O3_Obs')], O3_revised))
...
def cal_R2(xtest, ytest = None, info = ''):
    if ytest is None:
        u = np.concatenate((xtest[:,var_dict.get('O3_Obs')], xtest[:,var_dict.get('O3_Sim')])).reshape((2,-1))
        R2 = np.corrcoef(u)[0,1] ** 2
    else:
        y_pred = model.predict(xtest)
        y_pred = scaler2.inverse_transform(y_pred)
        O3_revised = ytest[:,var_dict.get('O3_Sim')] - y_pred.reshape(len(xtest),)
        v = np.concatenate((ytest[:,var_dict.get('O3_Obs')], O3_revised)).reshape((2,-1))
...
def cal_NMB(xtest, ytest = None, info = ''):
    if ytest is None:
        NMB = np.sum(xtest[:,var_dict.get('O3_Sim')] - xtest[:,var_dict.get('O3_Obs')]) / np.sum(xtest[:,var_dict.get('O3_Obs')])
    else:
        y_pred = model.predict(xtest)
        y_pred = scaler2.inverse_transform(y_pred)
        #bias=sim-obs,O3_revised=sim-bias
        O3_revised = ytest[:,var_dict.get('O3_Sim')] - y_pred.reshape(len(xtest),)
        NMB = np.sum(O3_revised - ytest[:,var_dict.get('O3_Obs')]) / np.sum(ytest[:,var_dict.get('O3_Obs')])
...
model.save("D:/project/data/BTH/O3/DNN_model.h5")