...
print(BTH_data.shape)
print(BTH_data[:,var_dict.get('PM2.5_Obs')].shape)
#计算scaler1, scaler2
...
DNN_model = load_model("D:/project/data/BTH/final_allvar/seed2/DNN_BTH_2.h5")
...
BTH_detail_pd = pd.DataFrame(np.concatenate((BTH_detail,BTH_data[:,var_dict.get('PM2.5_Obs')].reshape((363,1))),axis=1),index=np.arange(1,len(datasetX)+1),columns=var_sele+['PM2.5_Obs'])
...
#写入excel
writer = pd.ExcelWriter("D:/project/data/BTH/final_allvar/seed2/contribution_analysis_temp.xlsx")
