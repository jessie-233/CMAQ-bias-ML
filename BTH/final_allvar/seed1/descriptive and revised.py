...
BTH_data_pd = pd.DataFrame(BTH_data,index=np.arange(1,364),columns=all_vars)#(363, 42)
writer = pd.ExcelWriter("D:/project/data/BTH/final_allvar/seed1/decriptive_analysis_temp.xlsx")
BTH_data_pd.to_excel(writer,'BTH_basic',float_format='%.2f')	#保留小数点后2位
...
model = load_model("D:/project/data/BTH/final_allvar/seed1/DNN_BTH_1.h5")
y_pred = model.predict(datasetX)
y_pred = scaler2.inverse_transform(y_pred).reshape(len(datasetX),) #(363,)
PM25_revised = BTH_data[:,var_dict.get('PM2.5_Sim')] - y_pred
PM25_Bias_revised = PM25_revised - BTH_data[:,var_dict.get('PM2.5_Obs')]

#BTH_PM25 sheet：第1列PM2.5_Obs，第2列PM2.5_Sim，第3列PM2.5_Bias，第4列PM2.5_Bias_Predict，第5列PM2.5_revised，第6列PM2.5_Bias_revised
BTH_pm = pd.DataFrame({'PM2.5_Obs':BTH_data[:,6],'PM2.5_Sim':BTH_data[:,12],'PM2.5_Bias':BTH_data[:,0],'PM2.5_Bias_Predict':y_pred,'PM2.5_revised':PM25_revised,'PM2.5_Bias_revised':PM25_Bias_revised},index=np.arange(1,364))
BTH_pm.to_excel(writer, 'BTH_PM25', float_format='%.2f')	#保留小数点后2位
writer.close()