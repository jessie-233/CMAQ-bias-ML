...
m_loc = [(129,99),(129,99),(139,100),(142,100),(134,100),(142,100),(138,102),(138,102),(140,102),(130,104),(140,102),(146,103),(146,103),(137,105),(140,106),(143,106),(127,106),(133,105),(143,106),(143,106),(127,106),(139,109),(133,110),(138,112),(138,112),(140,112),(140,112),(138,112),(140,112),(136,115),(136,115),(136,115),(141,115),(141,115),(134,117),(137,117),(137,117),(146,116),(134,117),(137,117),(131,119),(140,120),(143,126),(143,126)]
...
for i in range(44):
    data[i,:,0] = (p_sim[p_loc[i][0],p_loc[i][1],1:,0] - p_obs[p_loc[i][0],p_loc[i][1],1:,3]) #PM2.5_Bias
    data[i,:,1] = (p_sim[p_loc[i][0],p_loc[i][1],1:,3] - p_obs[p_loc[i][0],p_loc[i][1],1:,0]) #NO2_Bias
    data[i,:,2] = (p_sim[p_loc[i][0],p_loc[i][1],1:,2] - p_obs[p_loc[i][0],p_loc[i][1],1:,1]) #SO2_Bias
    data[i,:,3] = (p_sim[p_loc[i][0],p_loc[i][1],1:,4] - p_obs[p_loc[i][0],p_loc[i][1],1:,2]) #O3_Bias
    data[i,:,4] = (p_sim[p_loc[i][0],p_loc[i][1],1:,5] - p_obs[p_loc[i][0],p_loc[i][1],1:,5]) #CO_Bias
    data[i,:,5] = p_obs[p_loc[i][0],p_loc[i][1],1:,3] #PM2.5_Obs
    data[i,:,6] = p_obs[p_loc[i][0],p_loc[i][1],1:,0] #NO2_Obs
    data[i,:,7] = p_obs[p_loc[i][0],p_loc[i][1],1:,1] #SO2_Obs
    data[i,:,8] = p_obs[p_loc[i][0],p_loc[i][1],1:,2] #O3_Obs
    data[i,:,9] = p_obs[p_loc[i][0],p_loc[i][1],1:,5] #CO_Obs
    data[i,:,10] = p_sim[p_loc[i][0],p_loc[i][1],1:,0] #PM2.5_Sim
    data[i,:,11] = (mete_sim[m_loc[i][0],m_loc[i][1],1:,0] - mete_obs[m_loc[i][0],m_loc[i][1],1:,1]) #RH_Bias
    data[i,:,12] = (mete_sim[m_loc[i][0],m_loc[i][1],1:,1] - mete_obs[m_loc[i][0],m_loc[i][1],1:,4]) #TEM_Bias
    data[i,:,13] = (mete_sim[m_loc[i][0],m_loc[i][1],1:,5] - mete_obs[m_loc[i][0],m_loc[i][1],1:,2]) #WSPD_Bias
    for j in range(1,365):  #WDIR_Bias
        WDIR_Bias = np.abs(mete_sim[m_loc[i][0],m_loc[i][1],j,6] - mete_obs[m_loc[i][0],m_loc[i][1],j,3])
        if WDIR_Bias > 180.:  
            data[i,:,14] = 360. - WDIR_Bias  
        else:
            data[i,:,14] = WDIR_Bias
    data[i,:,15] = (mete_sim[m_loc[i][0],m_loc[i][1],1:,4] - mete_obs[m_loc[i][0],m_loc[i][1],1:,0]) #PRE_Bias
    data[i,:,16] = mete_obs[m_loc[i][0],m_loc[i][1],1:,1] #RH_Obs
    data[i,:,17] = mete_obs[m_loc[i][0],m_loc[i][1],1:,4] #TEM_Obs
    data[i,:,18] = mete_obs[m_loc[i][0],m_loc[i][1],1:,2] #WSPD_Obs
    data[i,:,19] = mete_obs[m_loc[i][0],m_loc[i][1],1:,3] #WDIR_Obs
    data[i,:,20] = mete_obs[m_loc[i][0],m_loc[i][1],1:,0] #PRE_Obs
    data[i,:,21] = mete_sim[m_loc[i][0],m_loc[i][1],1:,2] #PBLH_Sim
    data[i,:,22] = mete_sim[m_loc[i][0],m_loc[i][1],1:,3] #SOLRAD_Sim

data = np.around(data, 2) #都保留2位小数
np.save("D:/project/data/BTH/dataset_abs.npy", data)