...
# 数据集制作
var_sele = ['PM2.5_Sim','NO2_Bias','SO2_Bias','O3_Bias','NO2_Obs','SO2_Obs','O3_Obs','RH_Bias','TEM_Bias','WSPD_Bias','WDIR_Bias','PRE_Bias','RH_Obs','TEM_Obs','WSPD_Obs','PRE_Obs','PBLH_Sim','SOLRAD_Sim'] #,'WDIR_Obs'
...
#加上WINDIR
#input_datasetx (16016,18)
def one_hot_WINDIR(input_datasetx):
    temp = np.where(np.logical_and(input_datasetx >= 0, input_datasetx < 90), 0, input_datasetx) #东北风0
    temp = np.where(np.logical_and(input_datasetx >= 90, input_datasetx < 180), 1, temp) #东南风1
    temp = np.where(np.logical_and(input_datasetx >= 180, input_datasetx < 270), 2, temp) #西南风2
    temp = np.where(np.logical_and(input_datasetx >= 270, input_datasetx < 360), 3, temp) #东南风3
    one_hots = to_categorical(temp)
    return one_hots #(xxx,4)

#print(dataset_winter[:,19].shape) #(3960,)
datasetX_winter = np.concatenate((datasetX_winter, one_hot_WINDIR(dataset_winter[:,19])), axis = 1) #增加列
datasetX_all = np.concatenate((datasetX_all, one_hot_WINDIR(dataset_all[:,19])), axis = 1) #增加列
#print(datasetX_winter.shape) #(3960, 22)
#print(datasetX_all.shape) #(16016, 22)
#print(one_hot_WINDIR(dataset_winter[:,19]))
