...
contribution_data = np.load("D:/project/data/BTH/exp/model3/BTH/3contribution_result_BTH.npy") #(100, 363, 19) 20、44都是nan，其余都有值
sensitivity_data = np.load("D:/project/data/BTH/exp/model3/BTH/3sensitivity_result_BTH.npy") #(100, 363, 19) 20、44都是nan，其余个别有nan

# #异常值nan检测
# for num in range(100):
#     for row in range(363):
#         if np.all(np.isnan(contribution_data[num,row,:])):    
#             print(num,row)
...
#写入excel
writer = pd.ExcelWriter("D:/project/data/BTH/exp/model3/BTH/model3_BTH.xlsx")
BTH_contribution_detail_pd.to_excel(writer,'3BTH_contribution_detail',float_format='%.1f')
BTH_sensitivity_detail_pd.to_excel(writer,'3BTH_sensitivity_detail',float_format='%.1f')
BTH_contribution_combine_pd.to_excel(writer,'3BTH_contribution_combine',float_format='%.1f')
BTH_sensitivity_combine_pd.to_excel(writer,'3BTH_sensitivity_combine',float_format='%.1f')
writer.close()
