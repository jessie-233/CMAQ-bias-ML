...
dataset_all = data_BTH.reshape((-1,43)) #(38115, 43)

test_data = np.concatenate((data_BTH[:,20:29,:],data_BTH[:,49:57,:],data_BTH[:,77:88,:],data_BTH[:,108:118,:],data_BTH[:,138:149,:],data_BTH[:,169:179,:],data_BTH[:,199:210,:],data_BTH[:,230:241,:],data_BTH[:,261:271,:],data_BTH[:,291:302,:],data_BTH[:,322:332,:],data_BTH[:,352:363,:]),axis=1).reshape((-1,43)) #(12915, 43)
...
dataset_X , dataset_y = get_xy_dataset(test_data)

#load model trained by BTH dataset
model = load_model("D:/project/data/BTH/DOMAIN_TRANS/DNN_tog_1.h5")
...
cal_rmse(dataset_X, test_data, 'all year revised')
...
cal_R2(dataset_X, test_data, 'all year revised')
...
cal_NMB(dataset_X, test_data, 'all year revised')
